...
c***  first executable statement
      ...
      !asitav
      !-----------
      xqc = (xv(jle,1,1)*0.75+xv(jtail1,1,1)*0.25) 
      yqc = (yv(jle,1,1)*0.75+yv(jtail1,1,1)*0.25) 
      !-----------
...
!$OMP DO ORDERED
     ...
     <       cfx,cfy,cm0,cpower)
        ...
        cl = - cfx * sa + cfy * ca
        cd =   cfx * ca + cfy * sa
        !amm cm =   cm0 - y0*cfx + x0*cfy
        cm =   cm0 - (yqc-yv(jle,1,1))*cfx + (xqc-xv(jle,1,1))*cfy
...
c     
      ...
      !asitav
      !-----------
      xqc = (xv(jle,1)*0.75+xv(jtail1,1)*0.25) 
      yqc = (yv(jle,1)*0.75+yv(jtail1,1)*0.25) 
      !-----------
...
10    continue
      ...
      if (iunst.eq.0.and..not.timespectral) then
        open(unit=10,file='cp.dat',status='unknown')
        do j=j1,j2
          write(10,*) x(j,k),zz(j)
        enddo
        close(10)
...
!amm        cmle = cmle + zz(j)*
!amm     &     (   0.5*(xv(jv+1,kv)+xv(jv,kv))*(xv(jv+1,kv)-xv(jv,kv)) +
!amm     &         0.5*(yv(jv+1,kv)+yv(jv,kv))*(yv(jv+1,kv)-yv(jv,kv)) )
        cmle = cmle + zz(j)*
     &     (   0.5*(xv(jv+1,kv)+xv(jv,kv)-xv(jle,1))*(xv(jv+1,kv)-xv(jv,kv)) +
     &         0.5*(yv(jv+1,kv)+yv(jv,kv)-yv(jle,1))*(yv(jv+1,kv)-yv(jv,kv)) )
        u=q(j,k,2)/q(j,k,1)
        v=q(j,k,3)/q(j,k,1)
...
!amm          cmlev = cmlev + zz(j)*
!amm     &    ( 0.5*(xv(jv+1,kv)+xv(jv,kv))*(yv(jv+1,kv) -yv(jv,kv))       +
!amm     &      0.5*(yv(jv+1,kv)+yv(jv,kv))*(xv(jv+1,kv) -xv(jv,kv)) )     -
!amm     &      0.5*(xv(jv+1,kv)+xv(jv,kv))*(xv(jv+1,kv) -xv(jv,kv))*syy(j)-
!amm     &      0.5*(yv(jv+1,kv)+yv(jv,kv))*(yv(jv+1,kv) -yv(jv,kv))*sxx(j)
          cmlev = cmlev + zz(j)*
     &    ( 0.5*(xv(jv+1,kv)+xv(jv,kv)-xv(jle,1))*(yv(jv+1,kv) -yv(jv,kv))       +
     &      0.5*(yv(jv+1,kv)+yv(jv,kv)-yv(jle,1))*(xv(jv+1,kv) -xv(jv,kv)) )     -
     &      0.5*(xv(jv+1,kv)+xv(jv,kv)-xv(jle,1))*(xv(jv+1,kv) -xv(jv,kv))*syy(j)-
     &      0.5*(yv(jv+1,kv)+yv(jv,kv)-yv(jle,1))*(yv(jv+1,kv) -yv(jv,kv))*sxx(j)
          u=q(j,k,2)/q(j,k,1)
          v=q(j,k,3)/q(j,k,1)
          um=q(j,k-1,2)/q(j,k-1,1)
          vm=q(j,k-1,3)/q(j,k-1,1)
